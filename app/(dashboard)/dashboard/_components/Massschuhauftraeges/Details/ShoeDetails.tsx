...
export default function ShoeDetails({
  ...
  const [linkerLeistenFileName, setLinkerLeistenFileName] =
    useState<string>("");
  const [rechterLeistenFileName, setRechterLeistenFileName] =
    useState<string>("");
          ...
          <div className="flex justify-center items-center gap-6">
            ...
            <div className="flex-1">
              <h2 className="text-2xl font-bold text-black mb-2">
                {orderDataForPDF.productName || shoe.name}
              </h2>
              <p className="text-lg text-black mb-2">
                Kunde:{" "}
                <span className="font-medium">
                  {orderDataForPDF.customerName || shoe.brand}
                </span>
              </p>
              <p className="text-base text-black mb-4">
                Bestellnr:{" "}
                <span className="font-bold">
                  {orderDataForPDF.orderNumber || "#123456789"}
                </span>{" "}
                &nbsp; Liefertermin:{" "}
                <span className="font-bold">
                  {orderDataForPDF.deliveryDate || "12.04.2024"}
                </span>
              </p>
      ...
      {showModal2 && (
        <CompletionPopUp
          ...
          onConfirm={async () => {
            ...
            if (!orderId) {
              toast.error(
                "Bestellungs-ID fehlt. Bitte versuchen Sie es erneut.",
              );
              setShowModal2(false);
              return;
            }
            ...
            try {
              ...
              // Bettungsdicke
              const bettungsdickeValue =
                getSelectedOptionLabel("bettungsdicke");
              ...
              // Laengsgewölbestütze
              const laengsgewoelbeValue =
                getSelectedOptionLabel("laengsgewoelbe");
              ...
              const zehenelementeValue =
                getSelectedOptionWithInputs("zehenelemente");
              formData.append(
                "Zehenelemente_Details",
                zehenelementeValue || "",
              );
              ...
              const korrekturBereichValue =
                textAreas["korrektur_bereich"] || "";
              formData.append(
                "eine_korrektur_nötig_ist",
                korrekturBereichValue,
              );

              // Spezielles_Fußproblem
              const fussproblemBettungValue =
                textAreas["fussproblem_bettung"] || "";
              formData.append("Spezielles_Fußproblem", fussproblemBettungValue);

              // Zusatzkorrektur_Absatzerhöhung
              const zusatzkorrekturenValue =
                getSelectedOptionWithInputs("zusatzkorrekturen");
              formData.append(
                "Zusatzkorrektur_Absatzerhöhung",
                zusatzkorrekturenValue || "",
              );
              ...
              const leistenPlatzhalterValue = getSelectedOptionLabel(
                "leisten_platzhalter",
              );
              formData.append(
                "Leisten_mit_ohne_Platzhalter",
                leistenPlatzhalterValue || "",
              );

              // Schuhleisten_Typ
              const schuhleistenTypValue =
                getSelectedOptionLabel("schuhleisten_typ");
              formData.append("Schuhleisten_Typ", schuhleistenTypValue || "");

              // Material_des_Leisten
              const leistenMaterialValue =
                getSelectedOptionLabel("leisten_material");
              formData.append(
                "Material_des_Leisten",
                leistenMaterialValue || "",
              );

              // Leisten_gleiche_Länge
              const gleicheLaengeValue =
                getSelectedOptionLabel("gleiche_laenge");
              formData.append(
                "Leisten_gleiche_Länge",
                gleicheLaengeValue || "",
              );
              ...
              const fussproblemLeistenValue =
                textAreas["fussproblem_leisten"] || "";
              formData.append(
                "Spezielle_Fußprobleme_Leisten",
                fussproblemLeistenValue,
              );
              ...
              const response = await sendMassschuheOrderToAdmin1(
                orderId,
                formData,
              );
