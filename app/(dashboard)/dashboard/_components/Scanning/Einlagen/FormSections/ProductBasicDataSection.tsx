...
interface ProductBasicDataSectionProps {
    einlagentyp: string;
...
export default function ProductBasicDataSection({
    einlagentyp,
    selectedEinlage,
    einlageOptions,
    showEinlageDropdown,
    ...
    return (
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 mb-6">
                        ...
                        {showEinlageDropdown && (
                            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                                {einlageOptions.map((option) => (
                                    <div
                                        key={option.name}
                                        onClick={() => onEinlageSelect(option.name)}
                        ...
                        {showUberzugDropdown && (
                            <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                                {uberzugOptions.map((option) => (
                                    <div
                                        key={option}
                                        onClick={() => onUberzugSelect(option)}
                                        className="px-3 py-2 hover:bg-gray-100 cursor-pointer text-sm"
                                    >
                                        {option}
                                    </div>
            ...
            {einlagentyp && onSpringerLogoClick && (
