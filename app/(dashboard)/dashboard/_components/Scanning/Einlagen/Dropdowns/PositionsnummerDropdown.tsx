...
interface PositionsnummerDropdownProps {
    ...
    vatCountry?: string;
    disabled?: boolean;
    /** Inline panel only (no trigger, no fullscreen overlay) — e.g. inside Dialog */
    variant?: 'default' | 'embedded';
    /** When variant is embedded, sync open state for focus + reset search/category */
...
export default function PositionsnummerDropdown({
    ...
    const isAustriaVatCountry =
        vatCountry === 'Österreich (AT)' || vatCountry === 'Austria (AT)';
    ...
    const panelActive = variant === 'embedded' ? embeddedActive : isOpen;
    ...
    const selectionPanel = (
                    <div
                        className={
                            variant === 'embedded'
                                ? 'bg-white w-full flex flex-col flex-1 min-h-0 overflow-hidden rounded-b-lg'
                                : 'bg-white rounded-lg shadow-lg w-full max-w-2xl max-h-[90vh] flex flex-col'
                        }
                        onClick={(e) => e.stopPropagation()}
                        onMouseDown={(e) => e.stopPropagation()}
                    >
                        {/* Modal Header — only in default fullscreen modal */}
                        {variant === 'default' && (
                        <div 
                        ...
                        {/* Search Bar */}
                        <div 
                            className="p-4 sm:p-6 border-b border-gray-200 shrink-0"
                            onClick={(e) => e.stopPropagation()}
                            onMouseDown={(e) => e.stopPropagation()}
                        >
                        ...
                        {value.length > 0 && (
                            <div 
                                className="border-t border-gray-200 p-4 sm:p-6 bg-gray-50 shrink-0"
                                onClick={(e) => e.stopPropagation()}
                                onMouseDown={(e) => e.stopPropagation()}
                            >
                                <div className="flex flex-col gap-2">
                                    {/* <div className="flex justify-between items-center pb-2 border-b border-gray-200">
                                        ...
                                        <span className="text-sm font-semibold text-gray-900">
                                            {value.length} Position{value.length === 1 ? '' : 'en'}
                                        </span>
                                    </div> */}
    ...
    if (variant === 'embedded') {
        if (!embeddedActive || disabled) return null;
        return (
