...
export const sendMassschuheOrderToAdmin2 = async (orderId: string, formData: FormData | any, isCourierContact: 'yes' | 'no' = 'yes') => {
    try {
        const response = await axiosClient.post(`/massschuhe-order/admin-order/send-to-admin-2-order/${orderId}/?custom_models=false&isCourierContact=${isCourierContact}`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
...
export const sendMassschuheCustomShaftOrderToAdmin2 = async (orderId: string, formData: FormData | any, isCourierContact: 'yes' | 'no' = 'yes') => {
    try {
        const response = await axiosClient.post(`/massschuhe-order/admin-order/send-to-admin-2-order/${orderId}?custom_models=true&isCourierContact=${isCourierContact}`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
