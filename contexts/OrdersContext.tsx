...
import {
  ...
  getSingleOrder,
  getAllOrders,
  groupOrderStatusUpdate,
  updateOrderPriority as updateOrderPriorityApi,
} from "@/apis/productsOrder";
...
const mapApiDataToOrderData = (
  apiOrder: ApiOrderData | null | undefined,
  selectedType?: string | null,
): OrderData | null => {
  ...
  return {
    ...
    erstelltAm: formatDate(
      werkstattzettel?.auftragsDatum || apiOrder.createdAt,
    ),
    fertiggestelltAm: formatDate(
      apiOrder.fertigstellungBis ||
        werkstattzettel?.fertigstellungBis ||
...
export function OrdersProvider({ children }: { children: ReactNode }) {
  ...
  const typeForApi =
    selectedType === "alle" ? "all" : selectedType || undefined;
  ...
  useEffect(() => {
    ...
    if (currentOrderId) {
      ...
      if (filteredOrders.length > 0) {
        ...
        setPrioritizedOrders(
          filteredOrders.filter((order) => order.priority === "Dringend"),
        );

        const foundOrder = apiOrders.find(
          (apiOrder) => apiOrder.id === currentOrderId,
        );
        if (foundOrder) {
      ...
      setPrioritizedOrders(
        mappedOrders.filter((order) => order.priority === "Dringend"),
      );
    } else {
  ...
  useEffect(() => {
    ...
    const orderExists = currentOrdersSnapshot.some(
      (order) => order.id === orderIdFromSearch,
    );

    if (orderExists && currentOrdersSnapshot.length > 1) {
      // If order exists and we have multiple orders, show only that order
      const filteredOrder = currentOrdersSnapshot.find(
        (order) => order.id === orderIdFromSearch,
      );
      if (filteredOrder) {
        ...
        setPrioritizedOrders(
          filteredOrder.priority === "Dringend" ? [filteredOrder] : [],
        );
        return;
      }
    }
      ...
      const fetchOrderById = async () => {
        try {
          ...
          if (response && response.success && response.data) {
            ...
            setPrioritizedOrders(
              mappedOrder.priority === "Dringend" ? [mappedOrder] : [],
            );
          }
        } catch (error) {
  ...
  const deleteBulkOrders = async (orderIds: string[]) => {
    ...
    setPrioritizedOrders((prevPrioritized) =>
      prevPrioritized.filter((o) => !orderIds.includes(o.id)),
    );
    triggerStatsRefresh();
  };
  ...
  const updateOrderPriority = async (
    ...
    setOrders((prev) => {
      ...
      setPrioritizedOrders(
        updatedOrders.filter((order) => order.priority === "Dringend"),
      );
  ...
  const updateBulkKrankenkasseStatus = (
    orderIds: string[],
    krankenkasseStatus: string,
  ) => {
    setOrders((prev) => {
