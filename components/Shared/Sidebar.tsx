...
export default function Sidebar({
    ...
    return [
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            label: "Automatisierte Nachrichten",
            href: "/dashboard/automatisierte-nachrichten",
          },
          {
            icon: Terminkalender,
      ...
      },
      {
        ...
        items: [
          {
            icon: FiDollarSign,
            label: "Kasse & Abholungen",
            href: "/dashboard/kasse",
          },
          {
            icon: BsCashStack,
            label: "Finanzen & Kasse",
            href: "/dashboard/finanzen-kasse",
          },
...

          {
            ...
            href: "/dashboard/warenwirtschaft",
          },
          {
            icon: TbUsers,
            label: "Mitarbeitercontrolling",
            href: "/dashboard/mitarbeitercontrolling",
          },
          {
            icon: TbActivityHeartbeat,
            label: "Krankenkasse",
            href: "/dashboard/einlagencontrolling",
          },
        ],
      },
      {
  ...
  const menuItems: MenuItem[] = useMemo(() => {
    ...
    return menuSections.flatMap((section, index) => {
      ...
      const items = rawItems
        .filter((item: any) => canShow(item.href, section))
        .map((item: any) => ({
          ...item,
      ...
      if (section.id === "4") {
        const kasseItems = items.filter(
          (item: any) =>
            item.href === "/dashboard/kasse" ||
            item.href === "/dashboard/finanzen-kasse",
        );
  ...
  return (
    <div
      className={`h-screen bg-white flex flex-col border-r border-gray-200 transition-all duration-300 w-80 ${isCollapsed ? "md:w-28" : "md:w-80"}`}
    >
      {/* Top Logo Section */}
      <div
        className={`py-5 flex items-center ${showLabels ? "px-3 justify-between" : "px-2 justify-between"}`}
      >
        <div
          className={`${showLabels ? "w-16 h-16" : "w-12 h-12"}  p-2 flex items-center justify-center overflow-hidden bg-[#61A175]/15 rounded`}
        >
          {user?.image ? (
            <Image
              src={
      ...
      <nav
        className={`mt-4 flex-1 overflow-y-auto px-3 ${isCollapsed ? "space-y-6" : "space-y-2"}`}
      >
        {featureLoading && features.length === 0 ? (
          ...
          menuItems.map((item) => {
            ...
            return (
              ...
              >
                <span
                  className={`relative flex items-center ${showLabels ? "px-5 justify-start" : "justify-center p-2"} py-1 rounded-lg transition-colors duration-200 ${
                    isActive
                      ? "bg-(--td-green,#61A175) text-white"
                ...
                >
                  ...
                  {item.type === "link" &&
                    ...
                    item.badge > 0 && (
                      <span
                        className={`${showLabels ? "ml-auto" : "absolute -top-1 -right-1"} bg-red-500 text-white text-xs font-bold rounded-full min-w-5 h-5 flex items-center justify-center px-1.5`}
                      >
      ...
      <div
        className={`border-t border-gray-200 ${showLabels ? "px-3 py-1" : "p-2"}`}
      >
        {showLabels ? (
