...
import {
  formatDate,
  getOrderStatusLabel,
} from "@/components/LastScans/Tables/utils";
import {
  DateRangeFilter,
  LastScanRow,
  OrderStatusFilter,
...
export default function LastScanTable({
  ...
  const [isDeleting, setIsDeleting] = React.useState<boolean>(false);
  const [apiLocations, setApiLocations] = React.useState<
    Array<{ description: string; address: string }>
  >([]);
    ...
    try {
      ...
      const processedData = data.map((item: any) => {
        ...
        if (item.latestScreener && item.latestScreener.createdAt) {
          ...
          Array.isArray(item.screenerFile) &&
          item.screenerFile.length > 0
        ) {
          const latest = item.screenerFile.reduce(
      ...
      if (meta) {
        ...
        setTotalPages(
          processedData.length < limit && page === 1
            ? 1
            : Math.max(1, page + (processedData.length === limit ? 1 : 0))
        );
        setTotalItems(processedData.length);
      }
    } catch (error: any) {
      if (error?.response?.data) {
        ...
        if (errorData.success === false && errorData.message) {
          ...
          toast.error(
            error.response.statusText || "Fehler beim Laden der Kundendaten"
          );
        }
      } else if (error?.message) {
        toast.error(error.message);
    ...
    const fetchLocations = async () => {
      try {
        ...
        const locations = allLocations
          ...
          .filter(
            (
              loc: { description: string; address: string },
              index: number,
              self: Array<{ description: string; address: string }>
            ) => self.findIndex((l) => l.address === loc.address) === index
          )
          .sort(
            (
              a: { description: string; address: string },
              b: { description: string; address: string }
            ) => a.description.localeCompare(b.description)
          );
  ...
  const handleDeleteClick = (row: LastScanRow) => {
    const customerName =
      `${row.vorname ?? ""} ${row.nachname ?? ""}`.trim() || "Kunde";
    setCustomerToDelete({ id: row.id, name: customerName });
    setDeleteModalOpen(true);
  };
    ...
    try {
      ...
      if (error?.response?.data) {
        ...
        if (errorData.success === false && errorData.message) {
          ...
          toast.error(
            error.response.statusText || "Fehler beim Löschen des Kunden"
          );
        }
      } else if (error?.message) {
        toast.error(error.message);
    ...
    return rows.filter((row) => {
      ...
      const matchesYear =
        ...
        (!Number.isNaN(createdDate.getTime()) &&
          createdDate.getFullYear().toString() === yearFilter);
      const matchesMonth =
        monthFilter === "all" ||
        (rowMonth !== null && rowMonth.toString() === monthFilter);
      return matchesYear && matchesMonth;
    });
  }, [rows, yearFilter, monthFilter]);
  ...
  const handleYearChange = (value: string) =>
    handleFilterChange(setYearFilter, value);
  const handleMonthChange = (value: string) =>
    handleFilterChange(setMonthFilter, value);
  const handleLocationChange = (value: string) =>
    handleFilterChange(setLocationFilter, value, false);
  const handleInsuranceChange = (value: string) =>
    handleFilterChange(setInsuranceFilter, value, false);
  const handleOrderChange = (value: OrderStatusFilter) =>
    handleFilterChange(setOrderFilter, value);
  ...
  const handleExport = () => {
    try {
      ...
      if (rowsToExport.length === 0) {
        toast.error(
          "Bitte wählen Sie mindestens eine Zeile zum Exportieren aus"
        );
        return;
      }
      ...
      const excelData = rowsToExport.map((row) => {
        const latestScreenerDate = row.latestScreener?.createdAt ?? null;
        const customerFullName = `${row.vorname ?? ""} ${
          row.nachname ?? ""
        }`.trim();
        const hasOrders = row.latestOrder || row.latestMassschuheOrder;
        let krankenkasseValue = "—";
        if (!hasOrders && row.billingType?.trim()) {
          krankenkasseValue =
            row.billingType === "krankenkasse"
              ? "Krankenkasse"
              : row.billingType === "privat"
              ? "Privat"
              : row.billingType;
        } else if (row.krankenkasse?.trim()) {
          krankenkasseValue = row.krankenkasse.trim();
        ...
        if (row.latestOrder) customerTypes.push("Einlagen");
        const kundentyp =
          customerTypes.length > 0 ? customerTypes.join(", ") : "—";
        ...
        const newestOrderLabel =
          orderEntries.length > 0 ? orderEntries.join(" | ") : "Kein Auftrag";
        const newestOrderDate = orderDates[0];
        ...
        return {
          ...
          Kostenträger: row.kostenträger?.trim() || "—",
          Kundentyp: kundentyp,
          "Neuester Scan": latestScreenerDate
            ? formatDate(latestScreenerDate)
            : "Kein Scan",
          "Neuester Auftrag": newestOrderLabel,
          Auftragsdatum: newestOrderDate ? formatDate(newestOrderDate) : "—",
          "Erstellt am": formatDate(row.createdAt),
      ...
      toast.success(
        `${rowsToExport.length} ${
          rowsToExport.length === 1 ? "Zeile" : "Zeilen"
  ...
  const yearOptions = [
    { label: "Alle Jahre", value: "all" as const },
  ...
  const locationOptions = [
    ...
    ...apiLocations.map((loc) => ({
      label: `${loc.address} - ${loc.description}`,
