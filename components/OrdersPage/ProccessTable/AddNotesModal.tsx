...
import { createNewNote } from '@/apis/productsOrder';
...
interface AddNotesModalProps {
    ...
    orderId: string | null;
    orderName?: string;
    /** API type for create: insole, milling_block, sonstiges, etc. */
    orderType?: string;
    onSuccess?: () => void;
}
...
export default function AddNotesModal({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSubmitting(true);
        try {
            const res = await createNewNote(orderType, { orderId, note: note.trim() });
            if (res?.success) {
                toast.success(res?.message || 'Notiz erstellt');
                setNote('');
                onSuccess?.();
                handleClose();
            } else {
